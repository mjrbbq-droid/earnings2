# scripts/extract_deep_signatures_gpt.py
...
from src.signatures_schema_deep import HomebuildersDeepSignature

PROMPT_VERSION = "hb_deep_v1"
...
def main() -> None:
    ...
    # IMPORTANT: if your pipeline now treats call_date as required for determinism,
    # it is best to avoid extracting deep signatures for calls missing call_date.
    call_ids = fetch_calls_for_signatures(conn, prompt_version=PROMPT_VERSION, model=model)
    if not call_ids:
        print("No calls pending deep signature extraction. Nothing to do.")
        return

    # Filter out calls missing call_date (keeps "latest" logic deterministic downstream)
    rows = conn.execute(
        f"""
        SELECT id
        FROM earnings_calls
    ...
    schema = TypeAdapter(HomebuildersDeepSignature).json_schema()
    schema = enforce_openai_strict_schema(schema)

    system_text = (
        "You are a senior homebuilders analyst. "
        "Extract a deep, structured signature focused on: margins, sales pace/demand quality, "
        "pricing power, incentives and rate buydowns, spec mix, backlog quality, competitive and inventory pressure, "
        "costs, cycle times, land/community growth, and guidance credibility. "
        "ALSO extract: sales_trend, volume_trend, price_vs_volume, and earnings guidance action "
        "(raised/cut/reaffirmed/withdrawn/no guidance), timing bias (front/back-half), and visibility. "
        "Use the transcript text only. Be conservative and choose 'unclear' if not supported. "
    ...
    with conn:
        for call_id in call_ids:
            ...
            def _do_request():
                return client.responses.create(
                    ...
                    text={
                        "format": {
                            "type": "json_schema",
                            "name": "homebuilders_deep_signature",
                            "schema": schema,
                            "strict": True,
                        }
                    },
            ...
            try:
                ...
                ok += 1
                print(f"OK: call_id={call_id} deep_signature_id={sig_id}")
    ...
    print("====")
    print(f"Deep signatures OK: {ok}")
    print(f"Deep signatures FAIL: {fail}")
    print(f"DB: {db_path}")
