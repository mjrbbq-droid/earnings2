...
SOURCES = [
    # --- Primary structured data ---
    {
        ...
        "limitations": "Finance/market-focused corpus. ZERO hits on police-specific keywords across 3,909 articles â€” wrong corpus for political/social events. No Russell 1000 endpoint (used S&P 500 as proxy).",
    ...
    },
    {
        ...
        "limitations": "Top-level 990 fields only â€” does NOT expose Schedule I line items via API (had to parse raw IRS XML for those).",
        "license": "Open data â€” ProPublica terms",
    },
    {
        ...
        "limitations": "Recipient-name fuzzy matching; subsidiary contracts may attribute to parent. Coverage 5 ticker-level connection errors (LDOS, LEN, LH, LVS, MGM, MSI) â€” worth retry pass.",
        "license": "Public domain (US Federal government data)",
    },
    {
        ...
        "limitations": "Foreign-listed issuers (UL/Unilever) file 20-F not 10-K, so EDGAR misses them. Initial run used ticker filter (broken) â€” corrected to CIK filter.",
    ...
    },
    {
        ...
        "limitations": "Reuters and Fast Company actively block archive.org â€” no snapshots available for those domains.",
    ...
    # --- Attempted but failed ---
    {
        ...
        "row_count": "0 â€” API timed out / rate-limited on most queries",
    ...
    # --- Curated inputs / methodology ---
    {
        ...
        "url": "Internal â€” see data/ directory in project",
        "cost": "Analyst time",
    ...
    },
    {
        ...
        "type": "Curated CSV",
        "provided": "Ticker â†’ corporate-foundation EIN mapping (21 corporate foundations + 9 DAF intermediaries). Built via ProPublica search + manual research.",
        "url": "data/reference/donor_foundations.csv",
        "cost": "Analyst time",
        "tables_populated": "donor_foundations",
        "row_count": "30 EINs",
        "limitations": "Some tickers (LULU, MSFT, AMZN US, HD) have no separate 501c3 foundation â€” give directly from corporate entity (no 990 filed for those flows).",
        "license": "Internal",
    },
    # --- Methodology / classification ---
    {
        ...
        "provided": "Signed -5.0 to +5.0 score per company derived from: reform-type weight + enforcement-type weight + status modifier + federal LE contract bonus. Sign: + = enforcement-leaning, âˆ’ = reform-leaning.",
    ...
    },
    {
        ...
        "limitations": "Pattern matching only â€” relies on canonical recipient names. Edge cases require manual review (e.g. ACLU classified as broad_cj_reform even though their specific programs vary).",
        "license": "Internal",
    },
]
...
def write_sources_sheet(wb, conn) -> None:
    ...
    phases = [
        ("Phase 0", "FMP news scrape + keyword-match against query_taxonomy â€” confirmed FMP corpus has zero police-specific hits across 3,909 articles."),
        ("Phase 1", "Claude (Opus 4.7 + web_search) investigates each ticker â†’ company_stance_investigation. Started with 3 watchlist tickers, expanded to S&P 500 priority sectors (254 tickers)."),
        ("Phase 2", "Taxonomy refinement (v3): split 'donations_police_reform' into adversarial / collaborative / re-entry / innocence sub-types. Re-investigated 30 anti-flagged tickers under refined taxonomy â€” 18 moved out of 'anti'."),
        ("Phase 3", "IRS Form 990 verification: located 21 donor foundations + 9 DAFs in IRS index, downloaded 24 monthly XML ZIPs (8.1 GB), parsed Schedule I/Part XV â†’ 29,443 grant-line records classified."),
        ("Phase 4A", "DAF outflow scan: parsed 21 DAF filings, found $1.99M flowing through DAFs to adversarial-anti-police orgs (BLM, Critical Resistance, ArchCity Defenders) â€” invisible from corporate foundation 990s alone."),
        ("Phase 4B", "USAspending federal-contracting layer: $11B+ in law-enforcement-agency contracts verified across 94 tickers â€” hardens 'enforcement-leaning' classification with primary-source dollar amounts."),
